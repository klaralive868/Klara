...
// Serialized: the rate-limit tests below share one file-wide, IP-scoped
// 'inquiry-ip' bucket (every local e2e run originates from 127.0.0.1, and
// this scope key is unique to this file's route, so no other spec file's
// traffic can add to it) — running this file's own tests concurrently
// (the framework default) would let them increment that shared bucket in a
// non-deterministic interleaving, breaking the exact-count assertions the
// rate-limit tests depend on.
test.describe.serial('public inquiry submission', () => {
	...
	test('an inquiry page is not reachable through a nonexistent organization slug', async ({
		page
	}) => {
	...
	// Regression: a fresh, never-before-seen email used to get its own
	// 5-request allowance no matter how many other emails the same requester
	// had already used — rotating emails was an unbounded-write bypass.
	// Deliberately doesn't assert on a precise iteration count (that would
	// hardcode this test's coupling to exactly how many requests the tests
	// above happen to make, which is fragile to maintain) — instead submits
	// a generous number of distinct-email requests and confirms rate
	// limiting kicks in at some point within them, which can only happen via
	// the IP-level bucket, since every email here is used at most once (each
	// individually far under its own 5-per-email allowance).
	test('rotating the submitted email does not bypass the IP rate limit', async ({ page }) => {
		let blocked = false;

		for (let i = 0; i < 15 && !blocked; i++) {
			await page.goto(`/book/${E2E_BOOKING_ORG_SLUG}/inquiry`);
			await page.getByLabel('Full name').fill('Rotating Email Visitor');
			await page.getByLabel('Email').fill(uniqueEmail(`inquiry-ip-limit-${i}`));
			await page.getByLabel('Where would you like to go?').fill('Somewhere new every time.');
			await page.getByRole('button', { name: 'Send inquiry' }).click();
			await expect(page.getByText(/Thanks!|Too many requests/)).toBeVisible();
			blocked = await page
