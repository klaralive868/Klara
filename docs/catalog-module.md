...
Admin-side product catalog management for the clothing vertical, plus a public storefront API and checkout. An organization's owner/manager/staff can create, edit, categorize, photograph, stock, and publish product listings ("Catalog Items"); an external site can browse published items and check out against real stock via `api/v1/catalog/{orgSlug}` (ADR-0008/ADR-0010).

Built across 8 tickets tracked under parent issue [#11](https://github.com/klaralive868/Klara/issues/11), all merged to `main`, plus the public checkout slice (ADR-0010). Backed by four ADRs: [0004](./adr/0004-catalog-categories-client-authored.md) (categories are client-authored, not a static registry), [0005](./adr/0005-catalog-images-private-no-public-read.md) (images started private, no public read access), [0008](./adr/0008-public-api-surface-per-module.md) (the shared public-API-per-module pattern), and [0010](./adr/0010-catalog-public-checkout.md) (images made public + checkout, resolving what 0005 deferred).
...
| **Catalog Item**  | One product listing: name, description, price, one Material Type, zero-or-more Categories, one-or-more Images (one primary), per-size Stock (or `unlimitedStock`), and a lifecycle status.                                                                                                                                                                                                                              |
| **Stock**         | Quantity of an item available at a given size (or, for sizeless types, the item as a whole). Recorded per `(item, size)`, independent of the Material Type's size _scheme_ — the scheme says which sizes exist; Stock says how many of each this item has. An item can instead be `unlimited_stock`, opting out of per-size tracking entirely — checkout is always accepted, an agent reconciles availability manually. |
| **Order**         | One checkout: a snapshot of what was bought (`items` jsonb — item id/name/size/quantity/unit price at time of purchase), delivery address, payment method, total, and a fulfillment status. Created only by the public checkout flow (`checkout_cart`), never directly by a dashboard user.                                                                                                                             |
...
| ------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `catalog_items`           | `20260721194500`, `unlimited_stock` added `20260730060000` | `organization_id` (default `current_organization_id()`), `name`, `description`, `price_cents`, `material_type`, `status`, `unlimited_stock` | `status` ∈ `draft` \| `published` \| `archived`. `material_type` references the code registry by key — not a DB table.                                                       |
...
| `catalog_item_stock`      | `20260722073313`                                           | `item_id`, `size` (nullable), `quantity`                                                                                                    | Two partial unique indexes cover the nullable-size case (see below). Ignored entirely for `unlimited_stock` items.                                                           |
| `orders`                  | `20260730060000`, `status` values updated `20260731010000` | `organization_id`, `customer_id`, `items` (jsonb snapshot), `payment_method`, `delivery_address`, `total_amount_cents`, `status`            | `status` ∈ `pending` \| `confirmed` \| `out_for_delivery` \| `cancelled`, forward-only (`pending`→`confirmed`→`out_for_delivery`, `cancelled` reachable from `pending`/`confirmed`). No inventory action on any transition — stock is already decremented at checkout. Closed-provisioning: no `authenticated` insert policy — only the service-role checkout path creates rows; `authenticated` does have `update` (status changes) from the dashboard. |
...
| `decrement_item_stock(item_id, size, quantity)`       | Single conditional `UPDATE ... WHERE quantity >= p_quantity`, returns whether it matched                                          | Atomicity via Postgres row-level locking on the statement itself, not check-then-write — two concurrent calls for the last unit correctly serialize to one winner.                                                                                                                                                                                         |
| `checkout_cart(...)`                                  | Calls `decrement_item_stock` per cart line (skipping `unlimited_stock` items), inserts the `orders` row                           | A function call shares its caller's transaction — every decrement plus the insert commit or roll back together, all-or-nothing across the whole cart.                                                                                                                                                                                                      |
...
- **Publish gating**: an item needs ≥1 category to be published. Enforced server-side in the `publish` action, checked _before_ any write — an unpublishable attempt leaves the item's existing tags and stock untouched. Not a DB constraint (cross-table rule, better suited to application logic — Standards §1).
...
- **Direct-Publish-without-Save**: the `publish` action reads `categoryIds` _and_ `stockQuantities` from its own form submission (not just prior DB state) — clicking Publish without clicking Save item first still persists whatever was just checked/entered, rather than silently discarding it.
...
| `GET /dashboard/orders`                  | Order list (status, customer, items, total), scoped to caller's org via RLS. No detail/status-change page yet — a follow-up.                                  |
...
| Layer                                            | Coverage                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Vitest (unit)                                    | Material Type registry invariants, `parseCatalogItemForm`/`parseStockQuantities` (dollars→cents rounding, malformed input, zero/negative quantities), public API/checkout parsing and shape (`public-catalog.test.ts`, `public-checkout.test.ts`).                                                                                                                                          |
| Vitest (integration, `npm run test:integration`) | `checkout-concurrency.integration.test.ts` — real local Postgres, not mocked: sufficient/insufficient-stock checkout, no-partial-mutation-on-rejection, exactly-one-winner under genuine concurrent checkouts for the last unit, unlimited-stock never rejects, cross-org isolation. First test of this kind in the repo (ADR-0010) — kept out of `test:unit` so that suite stays hermetic. |
...
## Public API and checkout (ADR-0010)

| Route                                      | Purpose                                                                                                                                                                                                                                                                                            |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /api/v1/catalog/{orgSlug}/items`      | Published items: `id, name, description, priceCents, materialType, images[], unlimitedStock, inStock, stockBySize`. Stock is booleans, never exact counts (ADR-0010).                                                                                                                              |
| `GET /api/v1/catalog/{orgSlug}/items/{id}` | Same shape, single item.                                                                                                                                                                                                                                                                           |
| `POST /api/v1/catalog/{orgSlug}/checkout`  | Body: `{name, email, phone, deliveryAddress, paymentMethod, items: [{itemId, size, quantity}]}`. Validates the whole cart, decrements stock atomically and all-or-nothing across every line (`checkout_cart`/`decrement_item_stock`), upserts the customer (`source: 'order'`), inserts the order. |

Same envelope/CORS/rate-limit conventions as Bookings' public API (ADR-0008): `{"data":...}` / `{"error":{"message":...}}`, origin-gated via `organizations.allowed_origins`, generic 404 for a bad `orgSlug` or an unpublished/nonexistent item.
