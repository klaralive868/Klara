...
| `orders`                  | `20260730060000`                                           | `organization_id`, `customer_id`, `items` (jsonb snapshot), `payment_method`, `delivery_address`, `total_amount_cents`, `status`            | `status` ∈ `pending` \| `confirmed` \| `fulfilled` \| `cancelled`. Closed-provisioning: no `authenticated` insert policy — only the service-role checkout path creates rows. |
